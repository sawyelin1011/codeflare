...
#
# Vault sync is advanced-mode-only (REQ-MEM-006 AC1, REQ-AGENT-005 AC2). In
# advanced mode the vault tree bisyncs to R2; in default mode we positively
# exclude it so it stays local-only and never round-trips. This rule is spread
# into RCLONE_FILTERS_COMMON at the position that precedes the graphify-out
# exclude (rclone first-match ordering, see comment at that line).
if [ "${SESSION_MODE:-default}" = "advanced" ]; then
    ...
    # global graph is rebuilt from it at boot) and the rendered viz (graph.html).
    # The per-run graph.json, chunks, labels, GRAPH_REPORT.md, cache/ and
    # manifest.json are regenerated by the next extraction and have no R2 value.
    # These three rules MUST precede "+ Vault/**" (rclone first-match ordering).
    VAULT_FILTER=(
        --filter "+ Vault/graphify-out/vault-graph.json"
        --filter "+ Vault/graphify-out/graph.html"
        --filter "- Vault/graphify-out/**"
        --filter "+ Vault/**"
    )
else
    VAULT_FILTER=(--filter "- Vault/**")
fi
...
RCLONE_FILTERS_COMMON=(
    ...
    # Codex's bundled system skills (imagegen, plugin-creator, skill-installer)
    # ship inside the codex binary and are re-extracted on launch, gated by
    # .codex-system-skills.marker. Not codeflare-managed, not user content;
    # excluding mirrors the .agents/ rationale (locally regenerated).
    --filter "- .codex/skills/.system/**"
    ...
    # rule (include in advanced mode, exclude in default mode) MUST precede
    # the global graphify-out exclude below: rclone uses first-match, so
    # without an explicit Vault rule here the vault tree (and its own
    # graphify-out/) would be caught by the exclude pattern. The vault is an
    # advanced-mode-only affordance (REQ-MEM-006 AC1, REQ-AGENT-005 AC2), so
    # in default mode we positively exclude it at this same ordering position
    # rather than letting it sync. SESSION_MODE defaults to 'default'.
    "${VAULT_FILTER[@]}"
    --filter "+ Uploads/**"
    --filter "+ Temporary/**"
