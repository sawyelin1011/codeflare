...
import { signSessionJWT, verifySessionJWT, SESSION_JWT_AUD } from '../../lib/session-jwt';
...
describe('session-jwt', () => {
  ...
  describe('verifySessionJWT', () => {
    ...
    // CF-005: a token lacking an aud claim must be REJECTED when an expectedAud
    // is supplied (no `!== undefined` escape hatch), but still accepted by the
    // 2-arg form that does not request an audience check.
    it('rejects a token with no aud claim when an expectedAud is required', async () => {
      const token = await signSessionJWT(
        { email: 'alice@example.com', sub: '12345', ghLogin: 'alice' },
        TEST_SECRET,
      );
      // No aud was minted into the payload.
      expect(await verifySessionJWT(token, TEST_SECRET, SESSION_JWT_AUD)).toBeNull();
      // Same token still verifies via the 2-arg form (no audience check).
      const payload = await verifySessionJWT(token, TEST_SECRET);
      expect(payload).not.toBeNull();
      expect(payload!.email).toBe('alice@example.com');
    });

    it('accepts a token whose aud matches the expectedAud', async () => {
      const token = await signSessionJWT(
        { email: 'alice@example.com', sub: '12345', ghLogin: 'alice', aud: SESSION_JWT_AUD },
        TEST_SECRET,
      );
      const payload = await verifySessionJWT(token, TEST_SECRET, SESSION_JWT_AUD);
      expect(payload).not.toBeNull();
      expect(payload!.aud).toBe(SESSION_JWT_AUD);
    });

    it('rejects a token whose aud does not match the expectedAud', async () => {
      const token = await signSessionJWT(
        { email: 'alice@example.com', sub: '12345', ghLogin: 'alice', aud: 'some-other-aud' },
        TEST_SECRET,
      );
      expect(await verifySessionJWT(token, TEST_SECRET, SESSION_JWT_AUD)).toBeNull();
    });
