...
20. `/sdd init` does not pre-create runtime tracking files at scaffold time. The layout-resolved triage file (`sdd/spec/triage.md` on the nested layout, `sdd/.review-needed.md` on the flat-legacy layout) is created lazily on first need by spec-reviewer, `/sdd clean`, or `/sdd init` Import-Mode triage. The doc-lane audit accumulator `documentation/.doc-coverage.md` is similarly lazy-created by doc-updater on first substantive finding. The `/sdd clean` execution audit lives in per-category commit bodies (recoverable via `git log --grep='\[sdd-clean\]'`), not in a dotfile.
