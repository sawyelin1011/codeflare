...
20. `/sdd init` touches empty `sdd/.review-needed.md`, `sdd/.coverage-report.md`, and `sdd/.last-clean-run.md` files at scaffold time with a single `_Awaiting first run._` placeholder, so the slot structure is visible from day one.
