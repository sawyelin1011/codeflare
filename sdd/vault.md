...
- `CONFIG.md` is a runtime contract, not user content. Its `libraries: - import: "[[!silverbullet.md/Library/Std/*]]"` directive triggers Library/Std federation on first browser open; without it the dashboard's `${query[[...]]}` blocks and in-page wikilink click handlers fail silently. This is why it lives in the always-overwrite tier (AC7) and not in the user-editable tier.
...
9. SilverBullet opens to the `index` page (the codeflare dashboard) on every Vault button click, via `indexPage: index` in `.silverbullet/config.yaml`. The README page is reachable from the dashboard via a link at the top.
