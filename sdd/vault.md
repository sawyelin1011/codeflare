...
**Acceptance Criteria:**
1. `/home/user/.obsidian_vault/` is included in `RCLONE_FILTERS_COMMON` with `+ .obsidian_vault/**`, placed BEFORE the existing `- **/graphify-out/**` exclude so the vault's own `graphify-out/` subdirectory rides along.
...
3. The skeleton (subdirectories + README + empty `graph.json` + SilverBullet config + best-effort Atlas plug) is created by `init_obsidian_vault()` in entrypoint.sh, idempotent so a returning session does not overwrite restored content.
4. `init_obsidian_vault()` runs AFTER `establish_bisync_baseline()` and BEFORE the daemon launch block, so we never write the empty skeleton over R2-restored content.
...
**Acceptance Criteria:**
1. `memory-agent-prompt.md` Step 4 writes the capture file at `/home/user/.obsidian_vault/raw/sessions/{ISO_TS}-{SID_SHORT}.md` using the YAML-frontmatter + Context/Decisions/Observations/References template.
...
2. `graphify-active-repo.sh` runs `flock /tmp/graphify-global.lock graphify global add <repo>/graphify-out/graph.json --as <basename>` the first time it sees a repo with a graph (idempotent via `graphify`'s hash-keyed manifest at `~/.graphify/global-manifest.json`).
3. The `/graphify` skill's commit step includes a `flock graphify global add` call so a fresh `graphify build` lands in the global graph.
4. All write sites (capture sonnet, vault-extract sonnet, active-repo hook, /graphify skill) serialise via `flock /tmp/graphify-global.lock` to prevent corrupted writes when multiple workflows race.
