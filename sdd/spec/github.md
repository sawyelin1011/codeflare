...
3. The panel renders beside the storage panel; its backend feature flag (`githubFeatureEnabled`) is on in every mode, and the dashboard renders the panel face whenever GitHub is enabled — with no session-tier entitlement ([REQ-GITHUB-007](#req-github-007-broaden-the-panel-gate-beyond-enterprise)). <!-- @impl: src/routes/github.ts::githubFeatureEnabled --> <!-- @impl: web-ui/src/components/github/GitHubPanel.tsx --> <!-- @impl: web-ui/src/components/Dashboard.tsx::effectiveFace --> <!-- @test: src/__tests__/routes/github.test.ts (status reports enabled in every mode) --> <!-- @test: web-ui/src/__tests__/components/GitHubPanel.test.tsx (renders nothing when status.enabled is false) -->
...
- The panel is available in every mode and renders whenever GitHub is enabled — there is no session-tier gate; it is the default right-column face on every session (connected or not). See [REQ-GITHUB-007](#req-github-007-broaden-the-panel-gate-beyond-enterprise).
...
<!-- Title retains "Broaden ... beyond enterprise" for anchor stability; the gate is now
     broadened all the way — the GitHub panel renders whenever GitHub is enabled, with no
     session-tier (advanced) entitlement. Originally enterprise-only, then advanced-in-any-mode
     (REQ-GITHUB-007 v1), now enabled-in-any-mode. -->

**Intent:** Make the GitHub repository panel + Storage browser available in every mode whenever GitHub is enabled — GitHub leads as the default right-column face for every session, with **no session-tier gate** — while keeping the OAuth connect/disconnect capability decoupled so a user can also connect GitHub from Guided Setup and the Settings accordion. Connect stays the additive OAuth flow ([REQ-GITHUB-001](#req-github-001-github-token-capture-and-storage)) — never a login-scope escalation, never a manually-pasted PAT. Enterprise is unchanged.

**Applies To:** User

**Acceptance Criteria:**

1. The panel backend (`GET /api/github/status.enabled`, `/repos`, `/clone`) is available in every mode (`githubFeatureEnabled` returns true); the frontend dashboard renders the GitHub panel face whenever GitHub is enabled (`githubStore.enabled`) with **no session-tier entitlement** — GitHub is the default right-column face on every session, connected or not. <!-- @impl: src/routes/github.ts::githubFeatureEnabled --> <!-- @impl: web-ui/src/components/Dashboard.tsx::effectiveFace --> <!-- @test: src/__tests__/routes/github.test.ts (status reports enabled outside enterprise; repos/clone reachable in non-enterprise) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (GitHub leads on every enabled session — a non-advanced session still defaults to the GitHub face with the storage flip control) -->
2. The GitHub panel and the Storage browser render in onboarding, default, and SaaS dashboards with the same labels and flip behavior as enterprise ([REQ-GITHUB-002](#req-github-002-github-panel-and-repository-listing)); GitHub leads for any enabled session regardless of session mode, and only a session with GitHub disabled shows the Storage face alone. <!-- @impl: web-ui/src/components/Dashboard.tsx::effectiveFace --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (forces the storage face when GitHub is disabled; GitHub leads when enabled regardless of session mode) -->
3. `GET /api/github/connect`, its callback, and `POST /api/github/disconnect` are gated by authentication only (any authenticated user, any non-enterprise mode/tier) — **not** by GitHub being enabled — so connect works from Guided Setup ([REQ-AUTH-015](authentication.md#req-auth-015-guided-onboarding-flow)) and the Settings accordion ([REQ-AGENT-018](agents.md#req-agent-018-push--deploy-credential-management-ui)) even where the dashboard panel is not shown. Only `status`/`repos`/`clone` follow GitHub being enabled. <!-- @impl: src/routes/github.ts --> <!-- @test: src/__tests__/routes/github.test.ts (connect reachable for a non-advanced authed user in non-enterprise) -->
...
**Intent:** GitHub leads the right column on every breakpoint. On mobile it is the single default face with a flip control to the R2 storage browser (and back); on tablet/desktop both stack as an anchoring split with no flip needed.

**Applies To:** User

**Acceptance Criteria:**

1. On mobile, GitHub is the default right-column face and the header flip control swaps to the R2 storage browser (and back) in place whenever GitHub is enabled. <!-- @impl: web-ui/src/components/github/GitHubPanel.tsx::GitHubPanel --> <!-- @impl: web-ui/src/components/Dashboard.tsx::effectiveFace --> <!-- @test: web-ui/src/__tests__/components/GitHubPanel.test.tsx (REQ-GITHUB-010: renders the mobile flip control only when onFlip is provided, and fires it) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (REQ-GITHUB-010: flips GitHub <-> storage when enabled and the flip controls are used) -->
2. On desktop and tablet both panels stack as an anchoring split: GitHub anchored to the top, Storage to the bottom. Each face is `flex: 1 1 0` capped at a JS-measured `max-height` equal to its natural content height (panel chrome + the scroller's `scrollHeight`, measured with the cap removed), so a short panel freezes at its content size while the larger one absorbs the freed flex space — `justify-content: space-between` drops both-short slack into the middle, and two full panels (neither cap binding before 50%) meet at 50/50. The panel expands to 80vh of the viewport (centered) so the split has room without going edge-to-edge. JS measures the per-panel caps (`measureLayout` → `measureNatural`); `panel-allocation.ts` only decides split-vs-flip. <!-- @impl: web-ui/src/styles/dashboard.css::.dashboard-panel-right --> <!-- @impl: web-ui/src/styles/dashboard.css::.panel-flip-face --> <!-- @impl: web-ui/src/lib/panel-allocation.ts::decidePanelLayoutMode --> <!-- @impl: web-ui/src/components/Dashboard.tsx::measureLayout --> <!-- @impl: web-ui/src/components/Dashboard.tsx::measureNatural --> <!-- @test: web-ui/src/__tests__/lib/panel-allocation.test.ts (decidePanelLayoutMode — splits on tablet/laptop viewports tall enough for two panels) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (applies the measured natural height as a face max-height and holds steady on a redundant observer callback; sizes the GitHub/Storage faces to their full list content via scrollHeight) --> <!-- coverage-gap: the flex redistribution of the freed space given the measured caps (space-between anchoring, 80vh expansion) is CSS layout with no jsdom geometry, not unit-assertable; the measured caps themselves ARE asserted by the tests above -->
...
6. The column collapses to a single panel with a flip control when the viewport is narrower than the mobile breakpoint or the column is too short to show both panels usably. <!-- @impl: web-ui/src/lib/panel-allocation.ts::decidePanelLayoutMode --> <!-- @impl: web-ui/src/components/Dashboard.tsx::measureLayout --> <!-- @test: web-ui/src/__tests__/lib/panel-allocation.test.ts (decidePanelLayoutMode — flips when the viewport is below the narrow breakpoint or the column is too short) -->
7. In single-panel (flip / mobile) mode the whole panel is content-sized and vertically centered (`.dashboard-panel:not(--expanded)` uses `flex: 0 0 auto` + `margin: auto 0` inside the stretched wrapper, so the slack splits evenly above and below rather than leaving an empty tail below), and the active face sizes to its content up to one shared viewport cap (both faces cap identically — mobile shows a single flip face at a time, so flipping never resizes the panel) with overflow scrolling inside the list — so a short panel (the connect card or a few repos) sits compact and centered instead of reserving the column. <!-- @impl: web-ui/src/styles/dashboard.css::.dashboard-panel --> <!-- @impl: web-ui/src/styles/dashboard.css::.dashboard-panel-right --> <!-- coverage-gap: panel height + centering is a pure CSS allocation (margin:auto + flex:0 0 auto + max-height cap), not unit-assertable in jsdom; same rationale as REQ-GITHUB-009 AC4 -->
...
<!-- Title retains "Mobile ..." for anchor stability; the disclosure now applies on
     EVERY breakpoint (desktop included), not only touch. -->

**Intent:** The GitHub panel's repository search is disclosed on demand on every breakpoint — hidden behind a magnify toggle so the repository list keeps its full whole-row viewport, revealed (and focused) on tap/click and hidden again on a second tap. The only touch-specific behaviour is scrolling the field above the on-screen keyboard once it opens.

**Applies To:** User

**Acceptance Criteria:**

1. On every breakpoint, the connected panel hides the search input by default and renders a magnify toggle to the left of the refresh control in the connected header. <!-- @impl: web-ui/src/components/github/ConnectedHeader.tsx::ConnectedHeader --> <!-- @impl: web-ui/src/components/github/GitHubPanel.tsx::GitHubPanel --> <!-- @test: web-ui/src/__tests__/components/GitHubPanel.test.tsx (hides search behind a magnify toggle left of refresh — touch case; desktop disclosure covered by AC4) -->
...
4. Desktop discloses the search the same way as touch — the magnify toggle renders and the search input stays hidden until the toggle is clicked (there is no always-on desktop search bar). <!-- @impl: web-ui/src/components/github/GitHubPanel.tsx::GitHubPanel --> <!-- @test: web-ui/src/__tests__/components/GitHubPanel.test.tsx (desktop also discloses search behind the magnify toggle) -->
