...
- **Vault** -- `/home/user/.obsidian_vault/`. Single source of truth for cross-session memory. Holds agent-written session captures (`raw/sessions/`) and user-curated notes (`notes/`, `raw/pasted/`). Rclone-bisynced to R2.
...
3. When triggered, the main agent spawns a background sonnet agent that reads the recent transcript and writes a markdown capture file into `/home/user/.obsidian_vault/raw/sessions/{ISO_TS}-{SID_SHORT}.md`.
...
5. The capture agent runs `graphify extract --file <file>` and `graphify global add ... --as vault` under `flock /tmp/graphify-global.lock` so the new content is queryable on the same turn it is written.
...
**Acceptance Criteria:**
1. In advanced mode, `/home/user/.obsidian_vault/` is included in rclone bisync via a `+` filter that precedes the global `**/graphify-out/**` exclude so vault graphify output still rides along.
...
3. The vault skeleton init (`init_obsidian_vault`) is idempotent and only creates subdirectories / config files when absent.
