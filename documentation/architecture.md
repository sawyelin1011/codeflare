...
2. **Re-reads `sleepAfter` from DO storage** (REQ-OPS-006 AC9). The class-field cache (`idleTimeoutPref`) can drift if the DO was hibernated and resumed, or if a `setBucketName` call updated storage after construction. Storage is always the authoritative source. When storage disagrees with the cache, the cache is updated via `setIdleTimeoutPref()`. If the storage read fails, the cached value is used as-is. Validated values must match `^(5m|15m|30m|1h|2h)$`; any other storage value is ignored with a warning.
3. Feeds the resolved `sleepAfter` string to `parseSleepAfterMs()`. **Fail-safe direction** (REQ-OPS-006 AC8): any unrecognized or malformed value — including empty string, `undefined`, or storage corruption — returns 7,200,000 ms (2h), the maximum supported timeout. A container that lives slightly longer than configured costs a few cents; a container that dies earlier destroys unpushed work.
4. Fetches `/activity` via `getTcpPort()` — reads `lastInputAt` (Unix timestamp of last PTY keystroke, tracked by the in-container terminal server) and computes `idleMs = Date.now() - (lastInputAt ?? containerStartedAt)`. If `idleMs > parseSleepAfterMs(idleTimeoutPref)`, writes KV status `'stopped'` and calls `stop('SIGTERM')` directly — this is the sole idle-hibernation path. If `/activity` returns non-OK or throws, the step is skipped and collectMetrics fails open (does NOT stop the container — a broken activity endpoint should not kill an active session).
5. Fetches `/health` via `getTcpPort()` — reads cpu/mem/hdd/syncStatus.
6. **Zombie DO detection**: when identifiers are missing (post-`destroy()`), returns early WITHOUT re-arming.
7. Writes metrics to KV session record (`session.metrics`) if container still running.
8. **Timekeeper usage ping** (SaaS mode only, when `SAAS_MODE=active` + `bucketName` + `userEmail` + `TIMEKEEPER` binding): increments `_usageSeconds` by 60, persists to DO storage, pings Timekeeper DO with `{ bucketName, sessionId, totalSeconds, email }`. If Timekeeper returns `quotaExceeded: true`, calls `stop('SIGTERM')` and returns without re-arming.
9. Re-arms the 60 s schedule if container still running.
...
**`destroy()` Override:** Clears `SESSION_ID_KEY`, `bucketName`, `workspaceSyncEnabled`, `tabConfig`, `fastStartEnabled` from DO storage and nulls `_bucketName`, `_sessionId`, `_r2AccessKeyId`, `_r2SecretAccessKey`, `_containerAuthToken`, `_openaiApiKey`, `_geminiApiKey`, `_githubToken`, `_cloudflareApiToken`, `_cloudflareAccountId`, `_encryptionKey` in memory, and resets `_sessionMode` to `'default'`. This prevents `onStop()` (triggered asynchronously by the eventual container exit) from resurrecting deleted sessions in KV. The override then sends `SIGTERM` and polls `ctx.container.running` for up to 25 s so the entrypoint trap can run the final `rclone bisync` before `super.destroy()` SIGKILLs the container as a fallback.
...
**`buildSetBucketNameBody` fails loudly on missing `sleepAfter`** (REQ-OPS-006 AC10). The helper in `src/routes/container/lifecycle.ts` that builds the `/_internal/setBucketName` request body throws if `sleepAfter` is absent. The `/start` route resolves the effective value from the user's stored preference before calling this helper (free tier: `'15m'`; paying users: `preferences.sleepAfter || '30m'`). A missing value at this layer would mean a code path skipped the resolution step, silently shipping a shorter timeout to a user who configured 2h. Throwing makes that silent substitution impossible. See `parseSleepAfterMs` fail-safe above for what happens when the value is malformed after reaching the DO.
...
**Session Stop Flow (user-initiated):** Sets KV status to `'stopped'`, calls `container.destroy()`. The DO's `destroy()` override clears `SESSION_ID_KEY`/`bucketName` from DO storage (prevents `onStop()` from resurrecting the deleted session), then sends `SIGTERM` and polls `ctx.container.running` for up to 25 s while the entrypoint trap runs the final `rclone bisync`, before falling back to `super.destroy()`'s SIGKILL. Both `batch-status` and `GET /:id/status` trust the `'stopped'` KV status to avoid waking the DO (exception: stale >5 minutes triggers probe).
