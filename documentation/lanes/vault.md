...
- `sweepOrphanVaultCaches(activeSessionIds)` -- called only after an authoritative `loadSessions()` fetch succeeds. Iterates every `vault-session-*` and `vault-session-*-idbs` entry in localStorage; for any sid not in `activeSessionIds`, runs the same deletion path. Catches sessions deleted via API in another tab or after a browser crash. Dashboard mount does not sweep, because it can see the initial empty store before the session list is known.
