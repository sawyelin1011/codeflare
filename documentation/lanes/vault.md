...
Second, `startVaultPrewarm()` mounts a hidden same-origin iframe at `/api/vault/:sid/.codeflare-bootstrap?codeflarePrewarm=1&prewarmId=...`. The bootstrap hop registers/configures the native service worker and preserves the prewarm query through the redirect. `injectVaultPrewarmBridge()` marks the runtime as headless without using SilverBullet's upstream `?headless` URL flag because that flag disables service worker registration. The bridge is injected into the generic shell because the service worker may serve the precached shell instead of a fresh Worker response. The button remains disabled until the bridge ready signal passes same-origin and prewarm-id validation. Timeout/error states stay disabled and retry in the background.

The landing page on every Vault button click is `Index.md` (the Codeflare dashboard), set by exporting `SB_INDEX_PAGE=Index` in the supervisor before launching the binary ([REQ-VAULT-012](../../sdd/spec/vault.md#req-vault-012-vault-button-render-and-dashboard-landing) AC3). The SilverBullet Go server hardcodes the default to lowercase `"index"` (`server/cmd/server.go:29`) and ignores any `indexPage` key in `.silverbullet/config.yaml` -- the env var is the only override. The dashboard now leads with `Notes/` and `References/` because those are the durable user-curated areas used by note-capture and reference workflows; broader recent-content widgets remain below. The README is one click away via a link at the top of the dashboard.
...
SilverBullet 2.8 ships full client-side IDB encryption via `EncryptedKvPrimitives` (`client/data/encrypted_kv_primitives.ts`). Activation requires three independent conditions checked in `client/boot.ts`:
...
On first browser open after a fresh vault, SilverBullet must build its browser IndexedDB stores and object index. Codeflare now does that work before the user can click: the Vault button remains disabled until the server probe succeeds and the hidden same-origin prewarm iframe emits the bridge ready signal ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-browser-prewarm-readiness-gating) AC1-AC6). Subsequent user clicks open a tab against an already-prepared browser cache instead of showing the indexing state first.
...
| `/api/vault/:sid/` returns 503 | SilverBullet supervisor not ready | The Vault button is disabled until `Layout.tsx` receives a 200 from `HEAD /api/vault/:sid/` and the browser prewarm completes. Wait for the header tooltip/status to leave the preparing/retrying state; the probe starts the hidden prewarm automatically once SilverBullet binds port 3030. |
...
| Vault button clickable during boot or while the first SilverBullet index is still running | UI is not gating on both readiness layers | The button should be disabled until `Layout.tsx` receives a 200 from `HEAD /api/vault/:sid/` and `startVaultPrewarm()` receives a same-origin ready message from the hidden SilverBullet client. If you see it clickable too early, check both the server probe and the `codeflare-vault-prewarm` iframe message path. |
