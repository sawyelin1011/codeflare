...
| `OAUTH_CLIENT_ID` | GitHub OAuth app client ID. Enables the Worker-managed GitHub OAuth flow; required for SaaS-mode login and for onboarding-mode `/login` GitHub sign-in. Its presence is what selects Direct GitHub OAuth over CF Access. **The OAuth App's authorization callback URL must match this deployment's own domain (`https://<domain>/auth/github/callback`).** A classic GitHub OAuth App allows only one callback URL, so each deployment domain (e.g. integration `codeflare.novoselec.ch` vs production `codeflare.ch`) needs its own OAuth App; sharing one App across domains makes GitHub bounce sign-in back to the registered domain. | - | yes | Wrangler secret | [REQ-AUTH-002](../../sdd/spec/authentication.md#req-auth-002-saas-mode-uses-direct-github-oauth), [REQ-AUTH-020](../../sdd/spec/authentication.md#req-auth-020-onboarding-mode-landing-integrated-login-and-access-request-flow) |
