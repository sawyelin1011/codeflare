...
| Enterprise Mode: agent uses wrong model or reports "no model available" | Pi `models.json` provider not registered, or `COPILOT_MODEL` / `PI_MODEL` env var not set when admin requires a specific model | When `ENTERPRISE_MODE=active`, entrypoint.sh registers each agent's gateway provider. If Pi shows no models, check `~/.pi/agent/models.json` exists and contains a `codeflare-enterprise` provider entry. Model selection is gateway-side; set `COPILOT_MODEL` or `PI_MODEL` deploy vars if the gateway requires a pinned model name. (Claude Code is not in the enterprise agent set; only Copilot, Pi, and Bash run under `ENTERPRISE_MODE=active`.) |
