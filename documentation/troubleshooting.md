...
| `graphify: command not found` in terminal (REQ-AGENT-023) | uv-installed shim PATH not picked up by the current shell | Confirm `/root/.local/bin` is on `PATH` (`echo $PATH`). The Dockerfile sets it globally via `ENV PATH=...`; if it is missing, the image is stale - redeploy. `which graphify` should return `/root/.local/bin/graphify`. |
