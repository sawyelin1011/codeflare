/**
 * Pure command-workflow data and assembly for codeflare-commands.ts.
 *
 * Extracted into a helper with no @earendil-works/pi-coding-agent dependency so the
 * injected-workflow content and the instruction assembly can be unit-tested in the backend
 * type-check + Workers test pool. codeflare-commands.ts itself imports the Pi package for its
 * command-registration types and so cannot be pulled into the backend tsc program.
 */

export const DEBUG_WORKFLOW = [
...
export const BRAINSTORM_WORKFLOW = [
...
export function commandInstructions(slash: string, workflow: string, input: string): string {
  return [slash, "", workflow, "", `User input: ${input}`].join("\n");
}

export function deployTarget(args: string): string {
  return args.trim() || "integration";
}
